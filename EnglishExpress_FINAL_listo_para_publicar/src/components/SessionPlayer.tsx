...
export const SessionPlayer: React.FC<SessionPlayerProps> = ({
  ...
  // Clave única para guardar el progreso parcial de ESTA sesión específica
  const progressKey = `session_progress_${subLevel}_${sessionNumber}`;

  // Initialize Exercises on mount: si hay progreso guardado de esta misma sesión
  // (el usuario salió a la mitad), lo retomamos exactamente donde se quedó.
  // Si no hay nada guardado, generamos la sesión desde cero como siempre.
  useEffect(() => {
    let resumed = false;
    try {
      ...
      if (saved) {
        ...
        if (parsed && Array.isArray(parsed.exercises) && parsed.exercises.length > 0) {
          ...
          if (parsed.disciplineHits) setDisciplineHits(parsed.disciplineHits);
          if (typeof parsed.correctCount === 'number') setCorrectCount(parsed.correctCount);
          if (parsed.vocabBonus) setVocabBonus(parsed.vocabBonus);
          resumed = true;
        }
      }
    } catch (e) {
      console.warn('No se pudo leer el progreso guardado, se inicia la sesión desde cero.', e);
    }
  ...
  // Cada vez que avanzas de ejercicio (o cambia tu progreso), lo guardamos
  // automáticamente. Así, si sales de la unidad antes de terminar, al volver
  // a entrar sigues exactamente donde te quedaste.
  useEffect(() => {
    if (exercises.length === 0 || sessionCompleted) return;
    try {
      ...
      console.warn('No se pudo guardar el progreso parcial.', e);
    }
  }, [exercises, currentIndex, disciplineHits, correctCount, vocabBonus, sessionCompleted]);

  // Al terminar la sesión por completo, borramos el progreso guardado
  // (ya no tiene caso "retomar" algo que ya se terminó).
  useEffect(() => {
    if (sessionCompleted) {
